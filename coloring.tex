...
\dots$ for all $v \in C(W)$ and that $h(\star,w,b^{n - 1}) = b^n$. To
check that $h$ is an homomorphism consider an edge $(d,v,e)
\rightarrow_a (d',v',e')$ in $L_n(w)$. We have to consider all the cases in
the definition of the relations in $L_n(w)$. The cases where $d = a u$
are obvious.
