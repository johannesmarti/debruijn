...
\begin{tikzpicture}
 ...
 \draw [<->] (a) edge node [left] {$0$} (b);
 \draw [<->] (a) edge [bend left=20] node [right] {$0$} (c);
 \draw [<->] (a) edge [bend left=30] node [right] {$0$} (d);
 \draw [<->] (b) edge node [right] {$0$} (c);
 \draw [<->] (b) edge [bend left=20] node [right] {$0$} (d);
 \draw [<->] (c) edge node [left] {$0$} (d);
...
But it can not color any $T_n$ because in any $T_n$ the path from
$01 \dots$ to $0 \dots$ is of odd length and there is no odd length path
in the pattern.
