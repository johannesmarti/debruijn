...
The pattern $P$ is \emph{construction deterministic} if $Q(P)$ is functional.
...
construction relation of $P$. Then $P$ is \emph{construction
$f$-deterministic } if $f(c) = f(c')$ whenever $((a,b),c) \in Q(P)$ and
$((a,b),c') \in Q(P)$.
...
\begin{lemma} \label{killer lemma}
 If $P$ is construction $f$-deterministic then there is no $k$ such that
$T_k \to P$.
\end{lemma}
